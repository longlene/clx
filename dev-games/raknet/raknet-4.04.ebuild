...
src_prepare() {
	...
	sed -i -e 's:add_subdirectory(DependentExtensions):#add_subdirectory(DependentExtensions):g' ${S}/CMakeLists.txt || die
	sed -i -e 's:INSTALL(TARGETS RakNetDynamic DESTINATION ${RakNet_SOURCE_DIR}/Lib/DLL):INSTALL(TARGETS RakNetDynamic DESTINATION lib):g' ${S}/Lib/DLL/CMakeLists.txt || die
        sed -i -e 's:INSTALL(TARGETS RakNetStatic DESTINATION ${RakNet_SOURCE_DIR}/Lib/LibStatic):INSTALL(TARGETS RakNetStatic DESTINATION lib):g' ${S}/Lib/LibStatic/CMakeLists.txt|| die
	sed -i -e 's:INSTALL(FILES ${ALL_HEADER_SRCS} DESTINATION ${RakNet_SOURCE_DIR}/include/raknet):INSTALL(FILES ${ALL_HEADER_SRCS} DESTINATION include/raknet):g' ${S}/Lib/LibStatic/CMakeLists.txt|| die
