...
pkg_setup() {
	if ! built_with_use =x11-libs/qt-4* accessibility || 
		! built_with_use =x11-libs/qt-4* qt3support; then
		eerror "=x11-libs/qt-4* must be built with accessibility and"
		eerror "qt3support prior to building ${PN}"
		die "Please re-emerge =x11-libs/qt-4 with proper support"
	fi
}
