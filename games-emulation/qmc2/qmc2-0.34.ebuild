...
SLOT="0"
KEYWORDS="amd64 ~x86"
IUSE="debug joystick opengl phonon +sdlmame sdlmess youtube"
...
RDEPEND="${DEPEND}
	sdlmame? ( =games-emulation/sdlmame-0.144 )
	sdlmess? ( =games-emulation/sdlmess-0.144 )
	x11-apps/xwininfo"
...
pkg_setup() {
	...
	# Validate USE flags
	if use youtube && ! use phonon; then
		die "You must set USE=phonon for youtube support."
	fi
	...
	use opengl && FLAGS="${FLAGS} OPENGL=1" || FLAGS="${FLAGS} ARCADE_OPENGL=0"
	use phonon || FLAGS="${FLAGS} PHONON=0"
	#use sqlite && FLAGS="${FLAGS} DATABASE=1"
	use youtube || FLAGS="${FLAGS} YOUTUBE=0"
}
...
src_prepare() {
	...
	sed -e 's%ifndef ARCH%ifdef ARCH%' -i Makefile
