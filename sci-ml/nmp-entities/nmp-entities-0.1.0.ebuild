# Copyright 2026 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

EAPI=8

DISTUTILS_SINGLE_IMPL=1
DISTUTILS_USE_PEP517=hatchling
PYTHON_COMPAT=( python3_{13..15} )

inherit distutils-r1

MY_PV="0.1.3-rc0"

DESCRIPTION="Entity management service for the NeMo Platform"
HOMEPAGE="https://github.com/NVIDIA-NeMo/nemo-platform"
SRC_URI="https://github.com/NVIDIA-NeMo/nemo-platform/archive/refs/tags/${MY_PV}.tar.gz -> nemo-platform-0.1.3_rc0.gh.tar.gz"

S="${WORKDIR}/nemo-platform-${MY_PV}/services/core/entities"

LICENSE="Apache-2.0"
SLOT="0"
KEYWORDS="~amd64"

RDEPEND="
	$(python_gen_cond_dep '
		>=dev-python/alembic-1.13.1[${PYTHON_USEDEP}]
		>=dev-python/asyncpg-0.31.0[${PYTHON_USEDEP}]
		>=dev-python/base58-2.1.0[${PYTHON_USEDEP}]
		>=dev-python/fastapi-0.100.0[${PYTHON_USEDEP}]
		>=dev-python/fastmcp-3.2.0[${PYTHON_USEDEP}]
		>=dev-python/greenlet-3.0.0[${PYTHON_USEDEP}]
		>=dev-python/lark-1.1.0[${PYTHON_USEDEP}]
		>=dev-python/psycopg2-2.9.10[${PYTHON_USEDEP}]
		>=dev-python/pydantic-2.0.0[${PYTHON_USEDEP}]
		>=dev-python/pydantic-settings-2.0.0[${PYTHON_USEDEP}]
		>=dev-python/sqlalchemy-2.0.0[${PYTHON_USEDEP}]
		>=dev-python/uvicorn-0.23.0[${PYTHON_USEDEP}]
		sci-ml/nemo-platform-sdk[${PYTHON_USEDEP}]
	')
	sci-ml/nmp-common[${PYTHON_SINGLE_USEDEP}]
"
BDEPEND=""
RESTRICT="test"
