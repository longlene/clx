...
PATCHES=(
	...
	"${FILESDIR}"/${P}-destdir-wrap_headers.patch
	"${FILESDIR}"/${P}-skip-version-py-install.patch
...
src_prepare() {
	...
	sed -i \
		-e 's|::fmt-header-only||' \
		-e '/^target_compile_definitions(fmt PUBLIC/d' \
		-e '/^target_compile_definitions(fmt-header-only INTERFACE/d' \
		c10/CMakeLists.txt \
	...
	# We deliberately build with an (almost) empty third_party directory
	# (everything is unbundled to system libs), so the upstream submodule
	# completeness check (git submodules aren't even fetched by our SRC_URI)
	# would always fail. Drop it.
	sed -i \
		-e '/--- Submodule initialization and verification ---/,/NCCL checkout/{/NCCL checkout/!d}' \
		cmake/PreBuildSteps.cmake \
		|| die

	if use cuda; then
		# dev-libs/cutlass installs its (source-only) python examples to
		# /usr/share/cutlass/examples/python regardless of USE flags; point
		# the vendored third_party/cutlass tree at it so FileMirroring.cmake's
		# CuTeDSL check finds the file.
		mkdir -p third_party/cutlass/examples || die
		ln -s "${ESYSROOT}"/usr/share/cutlass/examples/python \
			third_party/cutlass/examples/python || die
	fi
	# Change libc10* path
	sed -i \
	...
	# Replace placeholder added by cpp-extension-multilib.patch
...
python_install() {
	# Mirror upstream's own wheel file-selection filter (pyproject.toml's
	# [tool.scikit-build.wheel], packages = ["torch", "torchgen",
	# "functorch"]) since we ship these via a plain directory copy instead
	# of scikit-build-core. Keep this list in sync with pyproject.toml's
	# exclude list on future version bumps.
	local -a exclude=(
		--exclude='*.a'
		--exclude='torch/lib64/**'
		--exclude='torch/csrc/**'
		--exclude='torch/nativert/**'
		--exclude='torch/headeronly/**'
		--exclude='torch/lib/libshm/**'
		--exclude='torch/lib/libshm_windows/**'
		--exclude='**/CMakeLists.txt'
		--exclude='functorch/docs/**'
		--exclude='functorch/examples/**'
		--exclude='functorch/benchmarks/**'
		--exclude='functorch/op_analysis/**'
		--exclude='torchgen/_autoheuristic/**'
		--exclude='torch/share/doc/**'
		--exclude='torch/share/cmake/fbgemm/**'
		--exclude='torch/share/cmake/kineto/**'
		--exclude='torch/share/cmake/sleef/**'
		--exclude='torch/share/cmake/XNNPACK/**'
		--exclude='torch/share/cmake/ittapi/**'
		--exclude='torch/share/cmake/dnnl/**'
		--exclude='torch/share/cpuinfo/**'
		--exclude='torch/lib/dnnl.lib'
		--exclude='torch/lib/kineto.lib'
		--exclude='torch/lib/libprotobuf.lib'
		--exclude='torch/lib/libprotobuf-lite.lib'
		--exclude='torch/lib/libprotoc.lib'
		--exclude='torch/lib/mimalloc-2.3/mimalloc.lib'
		--exclude='torch/lib/cmake/**'
		--exclude='torch/cmake/**'
		--exclude='torch/lib/pkgconfig/**'
		--exclude='*.bzl'
		--exclude='*.ipynb'
		--exclude='*.png'
		--exclude='*.sh'
		...
		--exclude='.gitignore'
		--exclude='*.vim'
		--exclude='torch/header_only_apis.txt'
		--exclude='torch/ao/pruning/_experimental/data_sparsifier/benchmarks/**'
		--exclude='torch/ao/pruning/_experimental/data_sparsifier/lightning/tests/**'
		--exclude='torch/ao/quantization/experimental/**'
		--exclude='torch/distributed/benchmarks/**'
		--exclude='torch/distributed/checkpoint/examples/**'
		--exclude='torch/distributed/examples/**'
		--exclude='torch/distributed/tensor/examples/**'
		--exclude='torch/fx/experimental/shape_inference/**'
		--exclude='torch/utils/benchmark/examples/sparse/**'
		--exclude='torch/utils/_strobelight/examples/**'
		--exclude='torch/fx/experimental/unification/LICENSE.txt'
		--exclude='torchgen/decompositions/gen_*.py'
		--exclude='torchgen/fuse/gen_*.py'
		--exclude='torchgen/shape_functions/gen_*.py'
		--exclude='torch/_export/serde/gen-cpp2/**'
		--exclude='torch/share/ATen/Declarations.yaml'
		--exclude='torch/script.h'
		--exclude='torch/library.h'
		--exclude='torch/extension.h'
		--exclude='torch/custom_class.h'
		--exclude='torch/custom_class_detail.h'
		--exclude='torch/include/fp16/*.py'
		--exclude='torch/include/google/protobuf/*.inc'
		--exclude='torch/utils/viz/*.js'
		--exclude='torch/distributed/tensor/debug/*.js'
		--exclude='.git'
		--exclude='__pycache__'
		--exclude='*.pyc'
	)
	mkdir -p "${D}$(python_get_sitedir)" || die
	rsync -a "${exclude[@]}" torch torchgen functorch \
		"${D}$(python_get_sitedir)"/ || die

	# Narrow re-inclusions from upstream's exclude list: these subtrees are
	# excluded wholesale above, but a few files inside them still ship.
	local f
	while IFS= read -r -d '' f; do
		mkdir -p "${D}$(python_get_sitedir)"/$(dirname "${f}") || die
		cp "${f}" "${D}$(python_get_sitedir)"/${f} || die
	done < <(
		find torch/nativert -name '*.py' -print0 2>/dev/null
		find torch/lib/libshm torch/lib/libshm_windows -name '*.h' -print0 2>/dev/null
	)
	if [[ -e torch/csrc/inductor/aoti_runtime/model.h ]]; then
		mkdir -p "${D}$(python_get_sitedir)"/torch/csrc/inductor/aoti_runtime || die
		cp torch/csrc/inductor/aoti_runtime/model.h \
			"${D}$(python_get_sitedir)"/torch/csrc/inductor/aoti_runtime/ || die
	fi

	mkdir -p "${D}"$(python_get_sitedir)/torch/bin || die
	mkdir -p "${D}"$(python_get_sitedir)/torch/lib || die
	mkdir -p "${D}"$(python_get_sitedir)/torch/include || die
	ln -s ../../../../../include/torch \
	...
	# torch._C is a thin stub (torch/csrc/stub.c) that forwards into
	# libtorch_python.so; its RPATH ($ORIGIN/lib) expects the latter as a
	# sibling under torch/lib/.
	ln -s ../../../../../$(get_libdir)/libtorch_python.so \
		"${D}"/$(python_get_sitedir)/torch/lib/libtorch_python.so || die
}
...
src_install() {
	...
	# torch/CMakeLists.txt installs the torch._C extension module at the
	# install prefix root (mapping to torch/ in a wheel); relocate it into
	# the actual site-packages/torch directory it belongs in.
	local _c_ext
	for _c_ext in "${D}"/usr/_C.*; do
		[[ -e ${_c_ext} ]] || continue
		mv "${_c_ext}" "${D}"$(python_get_sitedir)/torch/ || die
	done
}
