...
FLASHMLA_COMMIT="df022ebafb88578eab9f0300606ee765608d8b5c"
FLASHINFER_COMMIT="bc29697ba20b7e6bdb728ded98f04788e16ee021"
SGLATTN_COMMIT="65c54cc5a6d29fee56036484c749bc5b8e00fd66"
...
src_prepare() {
	...
	# Drop dead include (nvls_device.hpp removed in mscclpp >= 0.7; sglang-kernel
	# only included it but never used any of its types).
	# Replace vendored mscclpp/fmt/cutlass FetchContent with system packages.
	eapply "${FILESDIR}/0001-drop-nvls_device-dead-include.patch"
	eapply "${FILESDIR}/0002-use-system-mscclpp-fmt-cutlass.patch"

	# Adapt allreduce code to mscclpp >= 0.9 API:
	#   connectOnSetup() → connect(EndpointConfig, rank, tag) returning shared_future<Connection>
	#   MemoryChannel src arg changed from void* to RegisteredMemory
	eapply "${FILESDIR}/0003-adapt-allreduce-to-mscclpp-0.9-api.patch"
	...
	eapply "${FILESDIR}/0004-adapt-cutlass-extensions-to-cutlass-4.5.patch"
