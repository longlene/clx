...
src_prepare() {
	...
	eapply \
		...
		"${FILESDIR}"/mkl-shared-libs.patch
...
src_configure() {
	...
	# FindCUDA's select_compute_arch.cmake includes sm_53 in its common list,
	# but CUDA 13+ dropped support for anything below sm_75. Override explicitly.
	use cuda && mycmakeargs+=( -DCUDA_ARCH_LIST="7.5;8.0;8.6;8.9;9.0" )
	cmake_src_configure
	wrap_python ${FUNCNAME}
}
