...
LEVEL_ZERO_COMMIT="35c037cdf4aa9a2e6df34b6f1ce1bdc86ac5422f"
...
"
#https://github.com/oneapi-src/level-zero/archive/${LEVEL_ZERO_COMMIT}.tar.gz -> level-zero-${LEVEL_ZERO_COMMIT}.tar.gz
...
src_prepare() {
	...
	eapply \
		...
		"${FILESDIR}"/${PV}-xptifw-dep.patch \
		"${FILESDIR}"/${PV}-system-unified-runtime.patch
	#"${FILESDIR}"/${PV}-system-umf.patch
	#"${FILESDIR}"/${PV}-system-l0.patch
	#mkdir "${WORKDIR}"/emhash-${EMHASH_COMMIT}/emhash && mv "${WORKDIR}"/emhash-${EMHASH_COMMIT}/*.hpp "${WORKDIR}"/emhash-${EMHASH_COMMIT}/emhash
	cmake_src_prepare
}
...
multilib_src_configure() {
	...
	local mycmakeargs=(
		...
		#-DUR_LEVEL_ZERO_SRC="${WORKDIR}"/level-zero-${LEVEL_ZERO_COMMIT}
	)
	local suffix=
...
multilib_src_install_all() {
	...
	mv "${ED}"${INTEL_DIR}/${libdir}/libsycl*.a "${ED}"/usr/${libdir}
