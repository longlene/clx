...
src_install() {
	...
	# The headers
	dodir /usr/include/${PF}
	insinto /usr/include/${PF}
	doins ./libsrc/include/*.hpp

	for headers_dir in \
	csg general geom2d grim gprim interface linalg \
	meshing ngsolve opti stlgeom visualization; do
		dodir /usr/include/${PF}/${headers_dir}
		insinto /usr/include/${PF}/${headers_dir}
		doins ./libsrc/${headers_dir}/*.hpp
	done

	if use opencascade; then
		dodir /usr/include/${PF}/occ
		insinto /usr/include/${PF}/occ
