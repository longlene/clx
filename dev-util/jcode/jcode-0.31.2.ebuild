...
declare -A GIT_CRATES=(
	[agentgrep]='https://github.com/1jehuang/agentgrep;1fc53630181548db7ad2c73ac8fb66969c04f854;agentgrep-%commit%'
...
src_prepare() {
	# jcode-base uses agentgrep v0.1.2 while jcode-app-core uses v0.1.3;
	# upgrade base to v0.1.3 so only one git commit is needed.
	sed -i 's|tag = "v0.1.2"|tag = "v0.1.3"|' crates/jcode-base/Cargo.toml || die
	perl -i -0pe 's/\[\[package\]\]\nname = "agentgrep"\nversion = "0\.1\.2"\n.*?\n\n//s' \
		Cargo.lock || die
	sed -i 's|"agentgrep 0\.1\.2"|"agentgrep 0.1.3"|g' Cargo.lock || die
	default
}
