...
SRC_URI="http://llvm.org/releases/${PV}/${P}.src.tar.gz
	...
	lldb? ( http://www.llvm.org/releases/3.4/lldb-3.4.src.tar.gz )
...
IUSE="clang lldb debug doc gold +libffi multitarget ncurses ocaml python
...
src_unpack() {
	...
	if use lldb; then
		mv "${WORKDIR}"/lldb-3.4 "${S}"/tools/lldb \
