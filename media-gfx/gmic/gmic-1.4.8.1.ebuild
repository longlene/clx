...
fftw board gimp bash-completion doc opencv"
...
-I/usr/include/ffmpeg -D__STDC_CONSTANT_MACROS"
	...
	if use gimp; then
		emake "CFLAGS+=${CXXFLAGS} ${UNIX_CFLAGS}" \
		"LDFLAGS+=${UNIX_LDFLAGS}" gmic_gimp \
			|| die "Compilation of gimp plugin failed"
	fi
}
...
src_install() {
	...
	if use doc; then
		dohtml -r -A shtml ../html/* || die "dohtml failed"
	fi
	if use gimp; then
		exeinto "$(pkg-config --variable=gimplibdir gimp-2.0)/plug-ins"
		doexe gmic_gimp || die "Installation of gimp plugin failed"
		einfo "You can find the gmic plugin for gimp in the menu \"Filters\"."
		einfo "It will be awesome for you artists !"
	fi
