...
#	export JAVAC="/usr/bin/ecj"
	...
	cp /usr/share/gnuconfig/config.* . || die
	...
	if use gjdoc; then
		...
		local antlr=/usr/share/antlr/lib/antlr.jar
		myconf="--with-antlr-jar=${antlr}"
	fi

	if use gconf; then
		myconf="${myconf} --enable-default-preferences-peer=gconf"
	else
		myconf="${myconf} --enable-default-preferences-peer=file"
	fi

	ECJ_JAR=/usr/share/eclipse-ecj-3.6/lib/ecj.jar
