# Copyright 2026 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

EAPI=8

DISTUTILS_USE_PEP517=setuptools
PYTHON_COMPAT=( python3_{13..14} )

inherit distutils-r1

DESCRIPTION="SQLAlchemy dialect integrated into Cloud Spanner database"
HOMEPAGE="
	https://pypi.org/project/sqlalchemy-spanner/
	https://github.com/googleapis/python-spanner-sqlalchemy
"
SRC_URI="https://github.com/googleapis/python-spanner-sqlalchemy/archive/refs/tags/v${PV}.tar.gz -> ${P}.gh.tar.gz"

LICENSE="Apache-2.0"
SLOT="0"
KEYWORDS="~amd64"

RDEPEND="
	>=dev-python/sqlalchemy-1.1.13[${PYTHON_USEDEP}]
	>=dev-python/google-cloud-spanner-3.55.0[${PYTHON_USEDEP}]
	dev-python/alembic[${PYTHON_USEDEP}]
"
#BDEPEND="
#	test? (
#	)
#"

EPYTEST_PLUGINS=()
distutils_enable_tests pytest

S="${WORKDIR}"/python-spanner-sqlalchemy-${PV}
