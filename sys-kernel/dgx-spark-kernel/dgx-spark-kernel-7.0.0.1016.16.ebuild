...
#
# Upstream itself is still pinned to the 6.17.0-1012.12 base (no newer commit
# as of this writing) - this version bumps only the Ubuntu source snapshot
# underneath to the newer 7.0 series (the same base sys-kernel/dgx-spark-kernel-bin
# now boots), applying upstream's same patch set on top. That combination is
# untested upstream; if the patches don't apply cleanly against 7.0's diff,
# that will surface here as a failed src_prepare, not a silent miscompile.
MY_KV_FULL="${PV%.*.*}"
MY_UBUNTU_PKG="${PV#${MY_KV_FULL}.}"
MY_KV_SERIES="${MY_KV_FULL%.*}"
# As with dgx-spark-kernel-bin, this upload is a noble (24.04) backport of the
# 7.0 series - upstream's version string carries a "~24.04.1" suffix that
# doesn't fit Gentoo's PV syntax, so it's appended separately below.
MY_BACKPORT="~24.04.1"

MY_LP_PKG="linux-nvidia-${MY_KV_SERIES}"
MY_LP_BASE="https://launchpad.net/ubuntu/+archive/primary/+sourcefiles/${MY_LP_PKG}/${MY_KV_FULL}-${MY_UBUNTU_PKG}${MY_BACKPORT}"
MY_ORIG="${MY_LP_PKG}_${MY_KV_FULL}.orig.tar.gz"
MY_DIFF="${MY_LP_PKG}_${MY_KV_FULL}-${MY_UBUNTU_PKG}${MY_BACKPORT}.diff.gz"
